...
def set_open_score(idx):
    if 0 <= idx < len(copies):
        ...
        copies[idx]['openScore'] = float(open_score)
        copies[idx]['totalScore'] = round((copies[idx].get('score', 0) + float(open_score)), 2)
        save_copies(copies)
        print(f"[NOTE OUVERTE] {copies[idx].get('name')} → {open_score}/5 | Total: {copies[idx]['totalScore']}/20")
        return jsonify({"ok": True, "totalScore": copies[idx]['totalScore']})
